...
export const DocumentList: React.FC<DocumentListProps> = ({ onLogout }) => {
  ...
  useEffect(() => {
    ...
    hubConnection.on('AddedToDocument', () => {
      fetchDocuments();
    });

    hubConnection.on('UserRoleChanged', (documentId: number, userId: number, newRole: string) => {
    ...
    hubConnection.on('UserRemoved', (documentId: number, userId: number) => {
    ...
    hubConnection.on('DocumentRenamed', (documentId: number, newName: string) => {
    ...
    return () => {
      ...
      hubConnection.off('AddedToDocument');
      hubConnection.off('DocumentRenamed');
      hubConnection.off('UserRoleChanged');
      hubConnection.off('UserRemoved');
    };
  }, []);
  ...
  const updateDocumentName = async (documentId: number, newName: string) => {
    ...
    try {
      ...
      await sendMessage('RenameDocument', [documentId, newName]);
      setEditDocumentInfo(prev => (prev ? { ...prev, name: newName } : prev));
  ...
  const updateUserRole = async (documentId: number, username: string, newRole: DocumentRole) => {
    try {
      ...
      await sendMessage('ChangeUserRoleInDocument', [documentId, user.userId, newRole]);
  ...
  const removeUserFromDocument = async (documentId: number, username: string) => {
    try {
      ...
      await sendMessage('RemoveUserFromDocument', [documentId, user.userId]);
  ...
  const addUsersToDocument = async (documentId: number, users: { username: string; role: DocumentRole }[]) => {
    ...
    try {
      await sendMessage('AddUsersToDocument', [{
